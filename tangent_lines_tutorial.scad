...
module Tutorial(tutorial)
{
    if (tutorial == 0)
    {
        ...
        translate([ 0, 0, 0.5 ]) color("Red") Line(C1, C2);
        ...
        Z1 = C1 + R2 * Vector;
        Z2 = C2 + R2 * Vector;
        translate([ 0, 0, 0.5 ]) color("Orange") Line(Z1, Z2);
    }
    else if (tutorial == 1)
    {
    ...
    else if (tutorial == 2)
    {
        ...
        translate([ 0, 0, 4 ])
        {
            color("LawnGreen", 0.5) polygon([ C1, P1[1], T1 ]);
            translate([ 0, 0, 0.5 ]) color("Orange", 0.8) polygon([ C2, P2[1], T1 ]);
        }
    }
    else if (tutorial == 3)
    {
        ...
        translate([ 0, 0, 4 ])
        {
            color("LawnGreen", 0.5) polygon([ C1, Q1[1], T2 ]);
            translate([ 0, 0, 0.5 ]) color("Orange", 0.8) polygon([ C2, Q2[1], T2 ]);
        }
    }
    else if (tutorial == 4 || tutorial == 5)
    {
        ...
        if (tutorial == 4)
        {
            translate([ 0, 0, 1 ])
            {
                color("Blue") translate(C10) circle(R10);

                color("Red") translate(C11) circle(R11);

                color("LawnGreen") translate(C12) circle(R12);

                color("Orange") translate(C13) circle(R13);

                translate([ 0, 0, 0.5 ]) color("Purple") translate(C14) circle(R14);
            }
            translate(C11) circle(R11);
            translate(C13) circle(R13);
            translate(C14) circle(R14);
        ...
        if (tutorial == 5)
            rotate_extrude() difference()
            {
                ...
                translate(C10) circle(R10);
                translate(C12) circle(R12);
            }
    }
    else if (tutorial == 6)
    {
        ...
        F3 = "Purple";
        TutorialCircle(C1, "C1", R1, "R1", N1, F1);
        translate([ 0, 0, 0.5 ]) TutorialCircle(C3, "C3", R3, "R3", N3, F3);
    ...
    else if (tutorial == 7)
    {
        ...
        TutorialCircle(C1, "C1", R1, "R1", N1, F1);
        translate([ 0, 0, 0.5 ]) TutorialCircle(C4, "C4", R4, "R4", N4, F4);

        L = TouchPoint(C1, R1, C4, R4);
        color("Red") translate([ 0, 0, 5 ]) Point(L, "L");
    }
    else if (tutorial == 8)
    {
    ...
    else if (tutorial == 9)
    {
        ...
        if ($preview)
        {
            color("Blue", 0.5) TutorialIntersectTouch2D(0);

            color("Red", 0.5) TutorialIntersectTouch2D(1);

            color("Black") translate([ 0, 0, 1.5 ]) TutorialIntersectTouch2D(4);

            color("Gold", 0.4) translate([ 0, 0, 2 ]) TutorialIntersectTouch2D(2);
        }

        color("Maroon") translate([ 110, 0, 0 ]) linear_extrude(5)
        {
            difference()
            {
                TutorialIntersectTouch2D(0);
                offset(-5) TutorialIntersectTouch2D(0);
            }

            difference()
            {
                TutorialIntersectTouch2D(3);
                offset(-5) TutorialIntersectTouch2D(3);
            }
        }
    }
}
...
module TutorialIntersectTouch2D(mode)
{
    ...
    Ptouch = [
        TouchPoint(Csmall[0], Rsmall, Cbig[0], Rbig), TouchPoint(Csmall[1], Rsmall, Cbig[0], Rbig),
        TouchPoint(Csmall[1], Rsmall, Cbig[1], Rbig), TouchPoint(Csmall[2], Rsmall, Cbig[1], Rbig),
        TouchPoint(Csmall[2], Rsmall, Cbig[2], Rbig), TouchPoint(Csmall[0], Rsmall, Cbig[2], Rbig)
    ];

    if (mode == 0)
    {
        for (i = [0:2])
            translate(Csmall[i]) circle(Rsmall);
    }
    else if (mode == 1)
    {
        for (i = [0:2])
            translate(Cbig[i]) circle(Rbig);
    }
    else if (mode == 2)
    {
        polygon(Ptouch);
    ...
    else if (mode == 4)
    {
        ...
        translate(Csmall[0]) difference()
        {
            circle(Rsmall + Rbig + line_width / 2);
            circle(Rsmall + Rbig - line_width / 2);
        }
        translate(Csmall[2]) difference()
        {
            circle(Rsmall + Rbig + line_width / 2);
            circle(Rsmall + Rbig - line_width / 2);
        }
...
module TutorialCircle(centre, centreName, radius, radiusName, name = "", fill_color = "")
{
    translate(centre)
    {
        ...
        // Height level 0.
        if (fill_color != "")
            color(fill_color, 0.2) circle(radius);
        ...
        if (name != "")
            color("Gray") translate([ -6, -5, 0.5 ]) text(name, size = 13, halign = "center", valign = "center");

        // The center point of the circle in black.
        // Height level 2.
        color("Black") translate([ 0, 0, 2 ]) Point([ 0, 0 ], centreName);

        // The edge of the circle in black.
        // Height level 2.
        color("Black") translate([ 0, 0, 2 ]) difference()
        {
        ...
        // Height level 2.
        if (radiusName != "")
            color("Black") translate([ 0, 0, 2 ]) rotate(90)
            {
                Line([ 2.5, 0 ], [ radius - 1.5, 0 ]);
                Line([ radius - 1.5, 0 ], [ radius - 6, -2 ]);
                Line([ radius - 1.5, 0 ], [ radius - 6, 2 ]);
                translate([ radius / 2 - 1, 1 ]) text(radiusName, size = 3, halign = "center");
            }
    }
}
...
{
    hull() for (pos = [ point1, point2 ]) translate(pos) circle(d = line_width);
}
...
module Point(coordinates, optional_text = "")
{
    ...
    {
        circle(2 * line_width);
        translate([ 1.8, 2.5 ]) text(optional_text, size = 3);
    }
}